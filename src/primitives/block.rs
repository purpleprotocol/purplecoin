...
///
/// ## Rules
/// * In total, there are 4 PoW chains, each with its own sector composed of 64 shards. Each
/// shard sector, as well as each individual shard can be processed concurrently.
///
/// * Each PoW block must declare one batch of 64 randomly sampled blocks to be appended
/// to the sector.
///
/// * As each block has a maximum size of `750kb`, the network will have to propagate a maximum
/// of `48mb` of transactions for a block to be placed in each shard. As nodes do not need to
...
/// 4 different chains, each responsible for a sector of 64 shards, each processed independently.
///
/// In this case, the system becomes exposed to the hashpower partition problem we described earlier: the total
/// hashpower percentage required to perform an attack on a single sector decreases from `51%` to `51/4 = 12.75%`
...
///
/// We require 3 runnerup blocks instead of 1 in order for a second round block to be mined. In this case,
/// you would need `12.75 * 4 = 51%` of the total hashpower in order to successfuly perform an attack.
...
/// bandwidth required for propagating a batch for a single node from `1.5mb/s` upload speed to
/// an average `300kb/s` upload speed for each sector. This is further reduced by running a node
/// in cluster mode, where the average upload speed is equal to `300kb/n` where `n` is the number
/// of nodes running in the cluster with a maximum of `n = 256`.
///
