...
use crate::group::{Codec, UnknownOrderGroup, ToInteger};
...
impl<G: UnknownOrderGroup, T: Hash + Clone> ProofOfCorrectness<G, T> {
  ...
  pub fn to_bytes(&self) -> Vec<u8> {
    match (&self.proof_added.proof, &self.proof_deleted.proof, &self.proof_added.witness, &self.proof_deleted.witness) {
      (added_proof, deleted_proof, added_witness, deleted_witness) if added_proof == deleted_proof && deleted_proof.Q == added_witness.0.value && added_witness.0.value == deleted_witness.0.value => {
        let mut buf = vec![0x00];
        buf.extend(self.proof_added.witness.to_bytes());
        buf
      }

      (added_proof, deleted_proof, added_witness, deleted_witness) if added_proof == deleted_proof && added_witness.0.value == deleted_witness.0.value => {
      ...
      (added_proof, deleted_proof, added_witness, deleted_witness) if added_proof != deleted_proof && added_proof.Q == added_witness.0.value && added_witness.0.value == deleted_witness.0.value => {
  ...
  pub fn from_bytes(bytes: &[u8]) -> Result<Self, &'static str> {
    ...
    match bytes[0] {
      0x00 => {
        ...
        Ok(Self { proof_added, proof_deleted })
      }
...
impl<G: UnknownOrderGroup, T: Eq + Hash + Clone + Send + Sync> Accumulator<G, T> {
  ...
  fn delete_(self, elem_witnesses: &[(T, Witness<G, T>)]) -> Result<(Self, Integer), AccError> {
    ...
    let (prime_product, acc_elem) = divide_and_conquer::<G, _>(
      (int(1), self.value),
      &prime_witnesses[..],
    )?;
  ...
  pub fn prove_membership(
    ...
    let (witness_accum, prod) = rayon::join(|| self.clone().delete(elem_witnesses), || elem_witnesses
      .par_iter()
  ...
  pub fn update_membership_witness(
    ...
    let (x, x_hat) = rayon::join(|| prime_hash_product(tracked_elems), || prime_hash_product(untracked_deletions));
    ...
    let (w_to_b, acc_new_to_a) = rayon::join(|| G::exp(&w.value, &b), || G::exp(&self.value, &a));
    Ok(Witness(Self {
      phantom: PhantomData,
...
impl<G: UnknownOrderGroup, T: Clone + Hash + Sync + Send + Eq> Witness<G, T> {
    ...
    T: PartialEq,
  {
    ...
    let (numerator, denominator) = rayon::join(|| prime_hash_product(witness_set), || prime_hash_product(witness_subset));
