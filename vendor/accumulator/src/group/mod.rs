...
pub trait Group: Clone + Debug + Eq + Hash + TypeRep + Send + Sync {
    ...
    type Elem: Clone + Debug + Eq + Hash + Sized + Send + Sync + Codec + ToInteger + From<Integer>;
    ...
    /// Encode to bytes
    fn to_bytes(x: &Self::Elem) -> Vec<u8> {
        x.to_bytes()
    }

    /// Decode from bytes
    fn from_bytes(bytes: &[u8]) -> Result<Self::Elem, &'static str> {
