...
lazy_static! {
  pub(crate) static ref INTERNAL_LRU: Vec<Mutex<LruCache<[u8; 32], Integer>>> = {
    ...
    for _ in 0..INTERNAL_LRU_SHARDS {
      v.push(Mutex::new(LruCache::new(NonZeroUsize::new(LRU_BYTES_PER_SHARD / 64).unwrap())));
    }
    v
  };
}
...
pub fn clear_cache_selectively<'a, T: Hash + 'a>(t: &'a [T])
  ...
  where [T]: IntoParallelRefIterator<'a>,
...
pub fn hash_to_prime<T: Hash + ?Sized>(t: &T) -> Integer {
  ...
  loop {
    ...
    if first_pass {
      let mut bytes = [0; 8];
      bytes.copy_from_slice(&hash[..8]);
      shard = jump_consistent_hash::hash(u64::from_le_bytes(bytes), INTERNAL_LRU_SHARDS) as usize;
      {
        let mut lru = (*INTERNAL_LRU)[shard].lock();
