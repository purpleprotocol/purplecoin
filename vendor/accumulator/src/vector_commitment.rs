...
mod tests {
    ...
    test_all_groups!(test_vc_basic, test_vc_basic_rsa2048, test_vc_basic_class,);
    fn test_vc_basic<G: UnknownOrderGroup>() {
        ...
        let to_open = [
            (
                int(2),
                Witness(Accumulator::<G, Integer>::empty().add(&[int(7), int(11)])),
            ),
            (
                int(7),
                Witness(Accumulator::<G, Integer>::empty().add(&[int(2), int(11)])),
            ),
        ];
        let open_verify = [(true, int(2)), (true, int(7))];
        let open_proof =
            VectorCommitment::open(&new_acc.clone(), &vc_acc_set, &non_members, &to_open).unwrap();
        assert!(new_acc
            .0
            .verify_membership_batch(&[int(2), int(7)], &open_proof.membership_proof));
